...
%% Plot
estimated_y(:,:) = (1/(pi*(1+x(4)+x(5))))*((1/(x(1)^2))*exp(-(X/x(1)).^2)+(x(4)/(x(2)^2))*exp(-(X/x(2)).^2)+(x(5)/(x(3)^2))*exp(-(X/x(3)).^2));
% for i=1:size(data,1)-1
%     delta_x(i)=X(i+1)-X(i);
% end
% E =sum(abs( (y(1:size(data,1)-1)-estimated_y(1:size(data,1)-1)).*X(1:size(data,1)-1).*delta_x'));
...
model_y = (1/(pi*(1+x(4)+x(5))))*((1/(x(1)^2))*exp(-(X/x(1)).^2)+(x(4)/(x(2)^2))*exp(-(X/x(2)).^2)+(x(5)/(x(3)^2))*exp(-(X/x(3)).^2));

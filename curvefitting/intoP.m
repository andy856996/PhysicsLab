...
global formula data
%% ------------------- 改這裡拉 ---------------------- %%%%%%%%%%%%%%
mertial = 'Te';%  Cr Ni3Al   NiTe2  TaBN  TaTe2   Te
load(['C:\Users\andy8\Desktop\HYY_DAT2\EUV_mask_' mertial '_100w_Dv500_500_200_PSF.mat']); %檔案路徑
formula = ...
    '(1/(pi*(1+x(3))))*((1/2*(x(1)^2))*exp(-(X/x(1)))+(x(3)/(x(2)^2))*exp(-(X/x(2)).^2));';%公式

title_name = mertial;% 要改材料名稱
title_formula = 'EG';% 要改
yourName = 'Ding'; % 你的名子

init_theta_energy= [   0.777008774	107.3222047	1.135860332
  ];
init_theta_lognsse=[  1.493890028	121.2769853	1.285950297
    ];
init_theta_log    =[ 1.836016056	127.2687588	0.912080741
   ];
